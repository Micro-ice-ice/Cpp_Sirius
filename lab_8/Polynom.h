...
public:
    ...
    //инициализация с помощью initializer list
    Polynom(initializer_list<int> list): Coefficients(list){}
    ...
    auto operator*(S s){
        ...
        using X = decltype(Coefficients[0] * s);

        X arr[size];

        for (int i = 0; i < size; i++)
        {
            arr[i] = Coefficients[i] * s;
        }

        return Polynom<X>(arr, size);
